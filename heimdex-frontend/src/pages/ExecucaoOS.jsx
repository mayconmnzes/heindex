    ...
    const fetchOsDetails = async () => {
        ...
        try {
            ...
            const savedResultsMap = osData.resultados?.reduce((map, result) => {
                map[result.itemTemplateId] = result; return map;
            }, {});
             osData.checklist?.itens?.forEach(item => {
                const savedResult = savedResultsMap ? savedResultsMap[item.id] : null;
                initialData[item.id] = { status: savedResult?.status || 'PENDENTE', observacao: savedResult?.observacao || '' };
            });
    ...
    return (
        <div className="main-content">
            ...
            <div className="container" style={{flexDirection: 'column', alignItems: 'flex-start'}}>
                ...
                {/* Seção Checklist (Com style={{width: '100%'}}) */}
                {os.checklist && (
                    <section className="list-section" style={{width: '100%'}}> 
                        <h3>Checklist: {os.checklist.nome}</h3>
                        <div className="table-container">
                            <table>
                                ...
                                <tbody>
                                    ...
                                    {(!os.checklist.itens || os.checklist.itens.length === 0) && ( <tr><td colSpan="3" style={{textAlign: 'center'}}>Checklist sem itens.</td></tr> )}
                                </tbody>
                            </table>
                        </div>
                    </section>
